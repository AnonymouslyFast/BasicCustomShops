...
public final class ShopManager {
    ...
    public void removeShop(Shop shop) {
        shops.remove(shop);
        dataService.removeSubShop(shop);
    ...
    public void addAdmin(ShopAdmin shopAdmin) {
        admins.put(shopAdmin.getPlayer().getUniqueId(), shopAdmin);
    }

    public void removeAdmin(ShopAdmin shopAdmin) {
        admins.remove(shopAdmin.getPlayer().getUniqueId());
    }

    public ShopAdmin getAdminByUUID(UUID uuid) {
    ...
    public String getInventoryName(@Nullable Shop shop, int page) {
        if (shop == null) { // When user wants to get the main shop name.
            return Messages.convertCodes(plugin.getConfig().getString("shop-title")) + " &7(" + page + ")";
        }
